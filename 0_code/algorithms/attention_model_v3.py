...
#                      #n_batch by (n_attention*n_attention_out*n_feat)
        ...
        hidden=self.dense_layer(attentions)        

        ##New (v3): Add Batch Normalization before activation
        hidden=self.batch_norm_layer(hidden)
...
#             ]) 
        ...
        ##TBD (v4): Shift Batch Normalization to after non-linearity as suggested in paper "All you need is a good init"
        eps=np.finfo(np.float32).eps        
        act=self.activation(hidden+eps)
        return act
