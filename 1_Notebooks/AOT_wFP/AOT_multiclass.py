...
if __name__ == "__main__":
	...
	assert np.isin([model_type], ["dense", "LS", "LSwFW", "LSwFW_ones"])[
		0], "Choose model type as one of: 'dense', 'LS', 'LSwFW', 'LSwFW_ones."
	...
	model_compile_params['loss']=CategoricalCrossentropy(label_smoothing=0.1)
	...
	for idx, label_idx in enumerate(multiclass_labels):
		...
		y_train = to_categorical(train_targets_scaled[valid_train_ind, idx])[:, 1:]
		y_test = to_categorical(test_targets_scaled[valid_test_ind, idx])[:, 1:]

		from sklearn.utils.class_weight import compute_sample_weight
		sample_weight = compute_sample_weight("balanced", y_train)
		n_cat=y_train.shape[1]
		model_setup_params['n_out']=n_cat
