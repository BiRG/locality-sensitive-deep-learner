...
if __name__ == "__main__":
    ...
    selected_Fweights = r['selected_Fweights']

    # Setup summary file
    folder_name = time.strftime("%y%m%d_LSwFW_models", time.localtime())
    ...
    for idx, (train_ind, test_ind) in enumerate(rkf.split(selected_features)):
        ...
        train_Fweights = selected_Fweights[train_ind, :]
        test_Fweights = selected_Fweights[test_ind, :]
        ...
        # Setup tf datasets
        train_ds = tf.data.Dataset.from_tensor_slices((np.hstack([train_features_scaled, train_Fweights]),
                                                       train_target_scaled
                                                       ))
        test_ds = tf.data.Dataset.from_tensor_slices((np.hstack([test_features_scaled, test_Fweights]),
                                                      test_target_scaled
                                                      ))
        ...
        results_train = model_eval(
            LSwFW_model,
            np.hstack([train_features_scaled, train_Fweights]),
            train_target_scaled,
            target_scaler,
            model_name="LSwFW",
            fold_idx=idx,
        ...
        results_test = model_eval(
            LSwFW_model,
            np.hstack([test_features_scaled, test_Fweights]),
            test_target_scaled,
            target_scaler,
            model_name="LSwFW",
            fold_idx=idx,
