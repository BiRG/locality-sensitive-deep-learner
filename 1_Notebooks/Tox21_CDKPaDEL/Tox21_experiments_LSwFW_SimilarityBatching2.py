	...
	for idx in [0]:
		...
		for label_idx, label in enumerate(labels):
			...
			summary_csv = os.path.join(checkpoint_path, "..", "..",  
				time.strftime("%y%m%d_LSwFWsummary.csv", time.localtime()))
		...
		# for label_idx, label in enumerate(labels):
			...
			with open(output_csv, 'a', newline='', encoding='utf-8') as f:
				writer = csv.writer(f, delimiter = ',')
				writer.writerow(["LSwFW_model", idx, label, "AUC", "Test", auc])
				writer.writerow(["LSwFW_model", idx, label, "f1", "Test", f1])
				writer.writerow(["LSwFW_model", idx, label, "acc", "Test", acc])
