...
# ADD ARGPARSER for setup kwargs??
	...
	for idx in [0]:
		...
		all_train_Fweights = np.ones_like(all_train_features_scaled)
		test_Fweights = np.ones_like(test_features_scaled)
		...
		for label_idx, label in enumerate(labels):
			...
			test_predict_class = tf.cast(tf.math.greater(test_predict, 0.5),
				dtype = tf.int32)
